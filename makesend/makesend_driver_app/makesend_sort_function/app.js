document.querySelector('.error_message__container__btns--close_btn').onclick =
  closeErrorIdList;
document.querySelector('.error_message__container__btns--copy_btn').onclick =
  copyAllErrorIds();
...
function createOptions(state) {
  ...
  let optionsHTML = Object.entries(parcels).reduce((options, parcel) => {
    if (parcel[0] in shipments) {
      options += `
            <option value='${parcel[0]}${String.fromCharCode(8218)} ${
        parcel[1].receiver_name
  ...
  if (data.selected) {
    ...
    searchResult.innerHTML = `
        <div id="sorting__search_result--list">
            ...
            <hr>
            <h3 id="rack_data" style="visibility: hidden">Rack: ${
              rack ? rack : 'Google Sheet has error'
            } - ${seq ? seq : 'Google Sheet has error'}</h3>
            <!-- <h3>Car Plate: ${carPlate} ${assignedDriver}</h3> -->
            <hr>
            <p class="${temp ? 'temp_control' : ''}">Temperature: ${
      temp ? 'Yes' : 'No'
    }</p>
...
function setupAllDOMs(state) {
  document.querySelector('.error_message__container__btns--close_btn').onclick =
    closeErrorIdList;
  document.querySelector('.error_message__container__btns--copy_btn').onclick =
    copyAllErrorIds(state);
  document.querySelector('#sorting__form').onsubmit = function (e) {
    e.preventDefault();
  };
...
function confirmTransfer(state = null) {
  if (state) {
    ...
    confirmBtn.onclick = async function () {
      ...
      setTimeout(function () {
        // simulate async request to confirm
        searchResult.innerHTML = ``;
      }, 3000);
    };
  }
